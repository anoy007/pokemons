...
export default function HomePage(props: any) {
  ...
  const pokemons = items?.map((p: any) => {
    return (
      <li key={p.id} className="h-50 w-50 p-2 m-2 border-box border-2 rounded-lg border-blue-300">
        <section>
          <img
            src={p.image}
            alt={p.name}
  ...
  return (
    <div className="mb-4 ml-60 mr-60 homepage">
      <ul className="flex flex-wrap place-content-center mb-2">{pokemons}</ul>
      <div className="pagination mb-4">
        {page !== 1 && <Link href="/?page=1">1</Link>}
        {page > 2 ? <Link href={`/?page=${page - 1}`}>{page - 1}</Link> : null}
